...
Parse.Cloud.afterDelete("PlaygroundFeed", function(request) {
    var feedPointer = {__type: "Pointer", className: "PlaygroundFeed", objectId: request.object.id};
    var query = new Parse.Query("PlaygroundLike");
    query.equalTo("likedFeed",feedPointer);
    query.each(function(like) {
        like.destroy();
    });
    query = new Parse.Query("PlaygroundComment");
        query.equalTo("playgroundFeedId",request.object.id);
